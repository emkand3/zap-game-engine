...
class DeactivatePhys:
    ...
    def act(self, pos):
        if self.condition_to_act(pos):
            ...
            #to True. If so, set that force to False.
            if self.type == 1: #spring
                if self.entity_state.spring_force[self.index] == True:
                    self.entity_state.spring_force[self.index] = False
            elif self.type == 2: #drag
                if self.entity_state.drag_force[self.index] == True:
                    self.entity_state.drag_force[self.index] = False
            elif self.type == 3: #gravity
                if self.entity_state.gravity_force[self.index] == True:
                    self.entity_state.gravity_force[self.index] = False
        return
